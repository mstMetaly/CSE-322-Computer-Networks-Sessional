...
#define MAX_CONGESTION_CONTROL_COUNT 2//task2added
...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_rreqCount(0),
      m_rerrCount(0),
      m_congestionControlCounter(0),//task2added
      ...
      m_lastBcastTime(Seconds(0))
{
    ...
    // Congestion Control Counter m_congestionControlCounter and 
    //Max Congestion Control Count set to MAX_CONGESTION_CONTROL_COUNT//task2added
}
    ...
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            .AddAttribute("MyRouteTimeout",
                          ...
                          MakeTimeChecker())
            .AddAttribute ("m_congestionControlCounter", "m_congestionControlCounter.",
                   UintegerValue (0),//task2added
                   MakeUintegerAccessor (&RoutingProtocol::m_congestionControlCounter),
                   MakeUintegerChecker<uint32_t> ())
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    //task2added
    if (m_congestionControlCounter > MAX_CONGESTION_CONTROL_COUNT){
        NS_LOG_DEBUG ("Congestion control counter > max congestion control count so RREQ ignored");
        return;
    }
  //endAdded
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    RrepHeader rrepHeader(/*prefixSize=*/0,
                          ...
                          1);//1 added for task2added --- Congestion control Flag 1 korlm  
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    //added for task2
    if(rrepHeader.GetCongestionControlFlag() == 1)
    {
        m_congestionControlCounter++;//Congestion Control Counter increased 1
    }
    ...
    RoutingTableEntry newEntry(
        ...
        rrepHeader.GetCongestionControlFlag());//task2Added
    RoutingTableEntry toDst;
    if (m_routingTable.LookupRoute(dst, toDst))
    {
    ...
    if (tag.GetTtl() < 2)
    {
        ...
        //task2Added
        if(m_congestionControlCounter > 0){
            m_congestionControlCounter--; // Congestion Control Counter -- (TTL exceeded)
        }
        //task2End
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    //task2Added
     if(m_congestionControlCounter > 0){
    m_congestionControlCounter--; // Congestion Control Counter decremented cause error means route removed
  }
  //task2End
}
